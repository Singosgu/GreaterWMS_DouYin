...
class OrderDownloadShopAccountItemFile(viewsets.ModelViewSet):
    """
        create:
            支持获取店铺数据
            下载资金流水明细文件(境内)
            通过download_id下载文件,download_id从order/downloadShopAccountItem接口获取，链接时效为1小时，失效后需重新调此接口获取新的链接
    """
    ...
    def create(self, request, *args, **kwargs):
        path = '/order/downloadShopAccountItemFile'
        params = self.params
