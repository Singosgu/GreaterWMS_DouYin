...
class OrderDownloadSettleItemToShop(viewsets.ModelViewSet):
    """
        create:
            支持获取店铺数据
            下载账单，生成downloadId(境内)
            商家后台，选择筛选条件，然后后台会下载账单数据，下载完后会生成一个download_id ，通过download_id下载文件见/order/downloadToShop接口
    """
    ...
    def create(self, request, *args, **kwargs):
        path = '/order/downloadSettleItemToShop'
        params = self.params
